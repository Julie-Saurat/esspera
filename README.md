...
[![Esspera Demo Video](https://user-images.githubusercontent.com/65171375/203348672-b311ec6a-9112-4620-8aa8-80a3c4555ba0.png)](https://youtu.be/bfOhn46MnRM)
...
Typically, large scale fields trials are conducted at the regional level through the cooperative efforts of farmer associations, universities, seed growers, governmental agencies, and others. These regional consortiums or organisations typically conduct performance trials for hundreds of varieties in multiples test sites, to account for varying local environmental conditions. They tend to publish the trials results for free, usually in a PDF format, but to keep ownership of it. From there, farmers are basically left on their own with minimal guidance to extrapolate which varieties would perform best on their land. The advice usually boils down to checking the varieties that performed best in at least the 3 test sites closest to your fields last year.

We believe there is room for improvements, especially considering how consequential seed selection is for productivity. This is where Esspera comes into play. It aims to help the farmer choose the best seeds to plant by automatically accounting for the impact of the forecasted environmental conditions and of the soil type at his specific location, and it does so without requiring significant additional efforts from the trials administrators. This last point is especially significant, as agricultural trials are already efforts-intensive endeavors requiring a lot of manual data preparation.

For the agronomist already preparing and handling the trial data for publication, Esspera is a simple and convenient tool to use. A ready-to-use template is provided to input the yields, trial year, and trial location of a seed variety. The solution will automatically populate the relative soil, temperature and precipitation relevant per year and trial location, a task the agronomist usually have to do manually. It will then automatically train and deploy a model of the relationship between these variables and the yield of the seed.

A pre-made, customizable front-end interface is provided. It can be easily added on the trials publisher website as a tool freely available to the farmers, who are already accustomed to visit the website to get trial results. This interface allows a farmer to ask for a personalized comparative prediction of seed varieties performance at his specific location for a given crop type. All he has to provide is a set of coordinates and a the crop type for which is wants to have a prediction. Esspera will automatically pull seasonal forecasting data from the Weather Company and soil data from SoilGrids, and interrogate the relevant seed yield models with those values to obtain a yield prediction. Results are then ranked and displayed in an easy to read manner to the farmer, along with the forecasted values used to establish the predictions.

We firmly believe that our solution will have a positive impact on the accuracy of seed selection for adopters, and thus on productivity and sustainability, the two key long terms variable of food security.
